  ...
  public:
    int numUniqueEmails(vector<string> &emails) {
        ...
        for (auto const &email : emails) {
            int at = email.find('@');
            string prefix = email.substr(0, at);
            string suffix = email.substr(at);
            int plus = prefix.find('+');
            prefix = prefix.substr(0, plus);
            ...
            if (map.count(suffix) > 0) {
                if (map[suffix].count(prefix) == 0) {
                    res++;
                }
            } else {
                map[suffix].insert(prefix);
                res++;
            }
        }
        return res;
